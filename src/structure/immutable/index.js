...
import { fromJS, isImmutable, isIndexed, List, Map } from 'immutable'
...
const structure: Structure<ImmutableMap<string, any>, ImmutableList<any>> = {
  ...
  getIn: (state: ImmutableMap<string, any> | ImmutableList<any>, field: string) =>
    isImmutable(state) ? state.getIn(toPath(field)) : plainGetIn(state, field),
  setIn,
  deepEqual,
  ...
  fromJS: jsValue =>
    fromJS(jsValue, (key, value) => (isIndexed(value) ? value.toList() : value.toMap())),
  keys,
  ...
  toJS: value => (isImmutable(value) ? value.toJS() : value)
}
