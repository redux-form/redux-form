...
The diagram below represents the simplified data flow. Note that in most cases you don't need to worry about the [action creators](http://redux-form.com/7.1.1/docs/api/ActionCreators.md/) for yourself, as they're already bound to dispatch for certain actions.
...
**NOTE:** The key used to pass the `redux-form` reducer should be named **`form`**. If you need a custom key for some reason see [`getFormState` config](http://redux-form.com/7.1.1/docs/api/ReduxForm.md/#-getformstate-function-optional-) for more details.
...
**NOTE**: `<Field/>` component is much more powerful. Apart from basic input types, it can take a class or a stateless component. When you're ready, go to the [docs](http://redux-form.com/7.1.1/docs/api/Field.md/#usage) to find out more.
...
You can now take it from here. We recommend to check out the [examples](http://redux-form.com/7.1.1/examples/). The common next steps could also be:
* setting the [initial form values](http://redux-form.com/7.1.1/examples/initializeFromState/),
* implementing the [validation](http://redux-form.com/7.1.1/examples/syncValidation/),
* creating dynamic forms with [arrays of fields](http://redux-form.com/7.1.1/examples/fieldArrays/).
